...
- **Evidence entry**: an object inside a note's `evidence` list, expected to
  ...
  `attributed_to` (string). Any of that may be missing, malformed, or
  misleading.
...
> into a single space, and strip leading/trailing whitespace. In Python:
> `" ".join(s.split())`.
>
...
Malformed entries are excluded from the R2 to R4 checks (there is nothing
valid to check); their quotes, when present, are unverified (R8).
...
`verified: True` means "this quote really appears in the cited segment" and
